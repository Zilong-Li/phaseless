...
#        sim data
#####################
setwd("/Users/zilong/Project/phaseless/misc")
library(inline)
source("ngsFatass.R")
source("fastPhase.R")
...
PI <- matrix(runif(M * C), ncol = C) # called alpha in fastPhase
PI <- PI / rowSums(PI)
par <- list(F = F, PI = PI)
#####################################
...
for (i in 1:10) {
  ...
  system.time(res <- lapply(1:N, function(x) forwardAndBackwards(x, e = logEmission, trans = trans, M = M, C = C, PI))) # p(Z|X,theta)
