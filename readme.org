...
The joint model now uses a structured start by default. First it makes =Q= uniform and =F= identical across ancestries and fits a shared-haplotype HMM. Genome-wide posterior cluster-occupancy profiles are then normalized per individual and grouped with multi-start farthest-point/k-means initialization. Distances to the group centroids define soft =Q= values, and each group's weighted cluster profile tilts a copy of the shared =F=. Short fixed-haplotype ancestry refinements select the highest-scoring candidate: observed likelihood for =--full-joint= and the penalized objective for the default regularized model.

After candidate selection, the default final fit is a regularized partial-joint model. It fixes the shared-haplotype estimates of =P= and =r=, continues to update =Q=, and represents ancestry-specific cluster frequencies as blockwise odds deviations from the dense shared baseline:

#+begin_example
F[k,s,c] proportional to F0[s,c] * exp(delta[k,block(s),c]).
#+end_example

The default deviation block contains 500 SNPs and receives ten effective shared-baseline refresh counts. Change these values with =--joint-f-block-size= and =--joint-f-block-shrinkage=. Blocks never cross an input HMM chunk. This reduces the ancestry-specific degrees of freedom in =F= while preserving site-specific shared haplotype frequencies. Use =--full-joint= to recover unrestricted sitewise =F= and to release =P= and =r= after initialization. =--random-init= and an explicit =--qfile= do not provide a learned shared baseline and therefore retain the legacy joint parameterization. The initialization scans are additional to =--iterations=.

The shared-haplotype stage now runs for at least 12 and at most 75 scans. It stops early only after three consecutive scans have both relative likelihood change below =1e-4= and normalized posterior-profile RMS change below =2e-3=. A final E-step produces profiles corresponding exactly to the saved shared parameters. When =--stitch-heuristics= is enabled, initialization reserves at least eight scans after its last possible heuristic perturbation before it can stop. The remaining defaults are 15 ancestry-refinement scans, three starts, and noise 0.05. These settings can be changed with the =--init-haplotype-*= options, =--init-ancestry-iterations=, =--init-restarts=, and =--init-noise=. Use =--random-init= to bypass the structured start for regression comparisons. An explicit =--qfile= also bypasses it so the supplied start is not overwritten. Posterior-driven initialization evaluates its shared-haplotype scans on the CPU so it can retain per-individual profiles; a requested CUDA backend resumes afterward.

Optional informative-SNP pruning for the initialization profile is enabled with =--init-profile-pruning=. Before the final profile E-step, it scores each SNP by the mutual information between its cluster-specific allele probabilities and posterior cluster occupancy. Within each 100-SNP block it retains at least five informative SNPs and then enough additional SNPs to preserve 95% of the block's information. Retained sites are reweighted to preserve the block's total profile contribution; blocks with no cluster-allele information contribute nothing. This pruning affects only the posterior cluster profile used to initialize =Q= and =F=: every SNP remains in the shared-haplotype HMM and final genotype imputation. Configure it with =--init-profile-block-size=, =--init-profile-information-fraction=, and =--init-profile-min-snps=. Without =--init-profile-pruning=, every SNP contributes equally as before.
...
The joint convergence defaults are an objective improvement per observation of =1e-5=, relative objective change of =2e-6=, parameter change of =5e-4=, and three consecutive stable iterations. Aitken rate and estimated gap are logged as diagnostics but do not control stopping because the extrapolation is discontinuous when the estimated rate crosses one.

For =--full-joint=, SqS3 acceleration compares the fully evaluated extrapolated state with the fully evaluated result of two ordinary EM maps. All of =Q,P,F,r= are snapshotted and restored together, a non-finite proposal falls back to ordinary EM, and the best observed-likelihood state is retained. The regularized partial-joint model instead uses ordinary block ECM: dense-F extrapolation would not preserve its blockwise odds parameterization. Its convergence and checkpoint objective adds the configured =Q= and =F= log-priors to the observed log likelihood.

The SqS3 step length balances blockwise mean-square changes in every active =Q=, =P=, =F=, and =r= parameter block, so the much larger site-level arrays do not dominate the extrapolation. The same active blocks are extrapolated together; =--aQ= remains available as a Q-only step-length override. The optimizer switches to ordinary EM when the underlying ordinary map approaches convergence, when at least 20% of the last 24 proposals were rejected, when the last 16 proposals delivered less than 5% mean realized improvement over ordinary EM, or for the final reserved part of the iteration budget. Testing the ordinary map prevents large successful extrapolations from masking convergence. The history-based rejection and efficiency conditions are activated only after the accelerated likelihood is already within a tolerance-scaled neighborhood. Only the non-accelerated finishing sequence is used for the consecutive likelihood-improvement and parameter-stability stopping test.
