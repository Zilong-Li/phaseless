...
During the first 20 ordinary EM scans, cluster labels are aligned every fourth iteration starting at iteration 4 by maximizing agreement between the per-individual cluster marginals implied by =Q= and =F=. All downstream cluster-indexed =P= and =F= parameters in the chunk are permuted together, and =P= is randomly reset within 20 SNPs of a changed boundary. Starting at iteration 6 and alternating every fourth iteration, posterior chromosome-copy occupancy is averaged over 100-SNP blocks; continuous intervals below 1% usage are initialized from a usage-weighted donor cluster with 80% donor emissions and 20% uniform noise. SqS3 acceleration starts after this warm-up and uses the remaining scan budget.
