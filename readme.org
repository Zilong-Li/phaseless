...
The CUDA implementation batches individuals according to available device memory. The regular build and command remain CPU-only; requesting =--gpu= from a CPU-only binary reports an actionable error instead of silently falling back.
