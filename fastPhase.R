    ...
    for(i in 1:N){    
        for(k1 in 1:C)
            for(k2 in 1:C){
                ...
                for(g1 in 0:1)
                    for(g2 in 0:1)
                        tmp <- tmp + like[[i]][1+g1+g2,]*(g1*F[k1,] + (1-g1)*(1-F[k1,]))*(g2*F[k2,] + (1-g2)*(1-F[k2,])) ## GL for G=g1+g2
                e[i,,k1,k2] <- tmp
            }
    }
    log(e)
...
transition <- function(d,PI,forward=TRUE){
    if(forward)
        dis <- c(1e20,d)
    else
        dis <- c(d,1e20)
...
forwardAndBackwards <- function(i,e,trans,M,C){
    # i is the indivudals
...
updatePar <- function(x,like,C,M,F,tol=1e-5){
    ...
    Ez <- Reduce("+",lapply(x,function(y) y$post))
