...
/*
 * genotype likelihood in VCF is log10 scale
 */
                                    ...
                                    const std::string & samples)
{
    ...
    genome->sampleids = vcf.SamplesName;
    genome->nsamples = vcf.nsamples;
    MyFloat1D gl; // store normal gl for one chunk
    Int1D pl; // store PL value
    genome->nsnps = 0;
    genome->nchunks = 0;
    int isnp{0};
    bool samechr{1};
    ...
    while(vcf.getNextVariant(var))
    {
        ...
        if(((++isnp % genome->chunksize == 0) || (samechr == false)) && isnp != 1)
        {
            ...
            for(int i = 0; i < genome->nsamples; i++)
            {
                ...
                {
                    glchunk[(size_t)i * im * 3 + 0 * im + j] =
                        gl[(size_t)j * genome->nsamples * 3 + i * 3 + 0];
                    glchunk[(size_t)i * im * 3 + 1 * im + j] =
                        gl[(size_t)j * genome->nsamples * 3 + i * 3 + 1];
                    glchunk[(size_t)i * im * 3 + 2 * im + j] =
                        gl[(size_t)j * genome->nsamples * 3 + i * 3 + 2];
                }
            }
            gl.clear();
            genome->gls.push_back(glchunk);
    ...
    { // add the rest into genome
        ...
        for(int i = 0; i < genome->nsamples; i++)
        {
            for(size_t j = 0; j < im; j++)
            {
                glchunk[(size_t)i * im * 3 + 0 * im + j] = gl[(size_t)j * genome->nsamples * 3 + i * 3 + 0];
                glchunk[(size_t)i * im * 3 + 1 * im + j] = gl[(size_t)j * genome->nsamples * 3 + i * 3 + 1];
                glchunk[(size_t)i * im * 3 + 2 * im + j] = gl[(size_t)j * genome->nsamples * 3 + i * 3 + 2];
            }
        }
        gl.clear();
        genome->gls.push_back(glchunk);
